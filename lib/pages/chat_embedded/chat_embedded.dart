...
class ChatEmbeddedPage extends StatefulWidget {
  final String roomId;
  final List<ShareItem>? shareItems;
  final String? eventId;

  const ChatEmbeddedPage({
    super.key,
  ...
  @override
  State<ChatEmbeddedPage> createState() => _ChatEmbeddedPageState();
}

class _ChatEmbeddedPageState extends State<ChatEmbeddedPage> {
  Room? room;
  bool isLoading = true;
  ...
  Future<void> getRoom() async {
    try {
      ...
      if (room == null) {
        await client.joinRoomById(
          widget.roomId,
        );

        await client.waitForRoomInSync(widget.roomId);
      ...
      Logs().w('Error while getting room', e, s);
      setState(() {
        isLoading = false;
      });
    } finally {
...
class ChatPageWithRoom extends StatefulWidget {
  ...
  @override
  ChatEmbeddedController createState() => ChatEmbeddedController();
}

class ChatEmbeddedController extends State<ChatPageWithRoom> with WidgetsBindingObserver {
  ...
  void _shareItems([_]) {
    ...
    final files = shareItems.whereType<FileShareItem>().map((item) => item.value).toList();
  ...
  void forwardEventsAction() async {
    ...
    final forwardEvents = List<Event>.from(selectedEvents).map((event) => event.getDisplayEvent(timeline)).toList();

    await showScaffoldDialog(
      context: context,
      builder: (context) => ShareScaffoldDialog(
        items: forwardEvents.map((event) => ContentShareItem(event.content)).toList(),
      ),
    );
    if (!mounted) return;
    setState(() => selectedEvents.clear());
  ...
  void scrollToEventId(
    ...
    final foundEvent = timeline!.events.firstWhereOrNull((event) => event.eventId == eventId);

    final eventIndex =
        foundEvent == null ? -1 : timeline!.events.filterByVisibleInGui(exceptionEventId: eventId).indexOf(foundEvent);
  ...
  void goToNewRoomAction() async {
    final newRoomId = room.getState(EventTypes.RoomTombstone)!.parsedTombstoneContent.replacementRoom;
    final result = await showFutureLoadingDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(
          child: ChatEmbeddedView(this),
        ),
        ValueListenableBuilder(
          valueListenable: _displayChatDetailsColumn,
          builder: (context, displayChatDetailsColumn, _) => !FluffyThemes.isThreeColumnMode(context) ||
